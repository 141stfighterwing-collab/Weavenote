...
export const login = async (usernameOrEmail: string, password: string): Promise<{ success: boolean; user?: User; error?: string }> => {
    ...
    try {
        ...
        // Security: Hardcoded default password removed. Use ADMIN_SETUP_PASS environment variable.
        if (usernameOrEmail === 'admin' && password === getEnvironmentKey('ADMIN_SETUP_PASS')) {
