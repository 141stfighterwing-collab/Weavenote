      ...
      <div className="mt-auto">
        ...
        <div className="flex items-center justify-end gap-2 pt-4 border-t border-black/5">
            ...
            <div className="relative">
                <button onClick={(e) => { e.stopPropagation(); setShowColorPicker(!showColorPicker); setShowCustomColorPicker(false); }} title="Change Note Color" aria-label="Change Note Color" className={`p-2 rounded-xl shadow-sm hover:scale-110 transition-transform ${isMatrix ? 'bg-[#39ff14]/10 text-[#39ff14]' : 'bg-white/50 text-slate-600'}`}>🎨</button>
                {showColorPicker && (
            ...
            <button onClick={(e) => { e.stopPropagation(); onEdit(note); }} title="Edit Note" aria-label="Edit Note" className={`p-2 rounded-xl shadow-sm hover:scale-110 transition-transform ${isMatrix ? 'bg-[#39ff14]/10 text-[#39ff14]' : 'bg-white/50 text-slate-600'}`}>✏️</button>
            <button onClick={(e) => { e.stopPropagation(); onDelete(note.id); }} title="Delete Note" aria-label="Delete Note" className="p-2 bg-rose-500/20 text-rose-700 rounded-xl shadow-sm hover:scale-110 transition-transform">🗑️</button>
            </div>
        </div>
      </div>
    </div>
