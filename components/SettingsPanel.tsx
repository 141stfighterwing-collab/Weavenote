...
import { 
    ...
    deleteUserAccount, updateUserRole,
          ...
          <div className="flex-1 p-8 overflow-y-auto custom-scrollbar bg-[#1a2333]">
              ...
              <div className="max-w-2xl space-y-6 animate-[fadeIn_0.2s_ease-out]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-black/20 border border-slate-700/50 rounded-xl">
                    ...
                    <code className="text-xs text-emerald-400 font-mono select-all truncate block">{currentUser?.uid || 'N/A'}</code>
                  </div>
                  <div className="p-4 bg-black/20 border border-slate-700/50 rounded-xl">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Email Address</p>
                    <p className="text-sm font-bold text-white truncate">{currentUser?.email || 'N/A'}</p>
                  </div>
                </div>

                <div className="p-6 bg-indigo-900/10 border border-indigo-500/20 rounded-2xl">
                    <h5 className="font-black text-white uppercase text-sm mb-4">Security Token (Password)</h5>
                    <form onSubmit={handlePasswordUpdate} className="space-y-4">
                      <input 
                        ...
                        className="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-lg text-white text-sm outline-none focus:ring-2 focus:ring-primary-500"
                      />
                      <button 
                        type="submit"
                        disabled={isLoading}
                      ...
                      >
                        {isLoading ? '⌛ Updating...' : 'Update Security Token'}
                      </button>
                    </form>
                    {passMsg && (
                      <div className={`mt-3 p-3 rounded-lg text-xs font-bold ${passMsg.type === 'success' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                        {passMsg.text}
                      </div>
                    )}
                </div>
                ...
                <div className="p-6 bg-[#0f172a] rounded-2xl border border-slate-700/50">
                  <div className="flex justify-between items-end mb-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Usage resets daily at 00:00 UTC.</p>
                    </div>
                    <span className="text-2xl font-black text-primary-500">{dailyAIUsage} / {DAILY_REQUEST_LIMIT}</span>
                  </div>
                  <div className="w-full h-3 bg-slate-800 rounded-full overflow-hidden">
                    <div className="h-full bg-primary-500 transition-all duration-1000" style={{ width: `${(dailyAIUsage / DAILY_REQUEST_LIMIT) * 100}%` }} />
                  </div>
                </div>

                <div>
                  <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4">Activity History</h4>
                ...
                <div className="bg-[#0f172a] rounded-2xl border border-slate-700/50 overflow-hidden">
                  <table className="w-full text-left text-[11px]">
                    <thead className="bg-slate-900/50 border-b border-slate-800">
                      <tr>
                        <th className="px-6 py-4 font-black uppercase text-slate-500 tracking-widest">User / Handle</th>
                        <th className="px-6 py-4 font-black uppercase text-slate-500 tracking-widest">Status</th>
                        <th className="px-6 py-4 font-black uppercase text-slate-500 tracking-widest">Role</th>
                        ...
                        <tr key={u.uid} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <select 
                                ...
                                className="bg-slate-800 border border-slate-700 text-white rounded text-[10px] px-2 py-1 font-bold outline-none focus:ring-1 focus:ring-indigo-500"
                            >
                          ...
                          <td className="px-6 py-4 text-right">
                             <div className="flex flex-wrap gap-2 justify-end">
                                <button 
                                    onClick={() => handleAdminResetTrigger(u.email)}
                                    className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded font-bold transition-all text-[9px] uppercase border border-slate-700"
                                >
                                    Reset PWD
                                </button>
                                {u.status === 'active' ? (
                                    <>
                                        <button 
                                            onClick={() => handleStatusChange(u.uid, 'suspended', 24)}
                                            className="px-2 py-1 bg-amber-600/20 hover:bg-amber-600 text-amber-500 hover:text-white rounded font-bold transition-all text-[9px] uppercase border border-amber-500/30"
                                        >
                                            24h Suspend
                                        </button>
                                        <button 
                                            onClick={() => handleStatusChange(u.uid, 'banned')}
                                            className="px-2 py-1 bg-rose-600/20 hover:bg-rose-600 text-rose-500 hover:text-white rounded font-bold transition-all text-[9px] uppercase border border-rose-500/30"
                                        >
                                            Ban
                                        </button>
                                    </>
                                    ...
                                    <button 
                                        ...
                                        className="px-2 py-1 bg-emerald-600/20 hover:bg-emerald-600 text-emerald-500 hover:text-white rounded font-bold transition-all text-[9px] uppercase border border-emerald-500/30"
                                    >
                                        Reactivate
                                    </button>
                                )}
            ...
            {activeTab === 'admin-logs' && (
              <div className="animate-[fadeIn_0.2s_ease-out]">
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 font-mono text-[11px] h-[500px] overflow-y-auto custom-scrollbar shadow-inner">
                  {systemLogs.length > 0 ? systemLogs.map(log => (
                    <div key={log.id} className="py-1 border-b border-white/5 group flex items-start gap-3">
