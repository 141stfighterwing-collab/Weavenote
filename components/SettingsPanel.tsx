...
service cloud.firestore {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-[fadeIn_0.2s_ease-out]">
      <div className="bg-surface-container-high border border-outline-variant rounded-2xl shadow-2xl w-full max-w-7xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="p-5 border-b border-outline-variant flex justify-between items-center bg-surface-container-low">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold text-on-surface uppercase tracking-tight flex items-center gap-2">
              <span className="p-1.5 bg-primary-container text-on-primary-container rounded-lg shadow-sm material-symbols-outlined text-[20px]">settings</span>
              System Control
            </h2>
          </div>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-surface-variant transition-colors text-outline hover:text-on-surface"><span className="material-symbols-outlined text-[20px]">close</span></button>
        </div>

        <div className="flex flex-1 overflow-hidden">
          {/* Sidebar */}
          <div className="w-64 border-r border-outline-variant bg-surface-container-low p-4 space-y-1.5 overflow-y-auto">
            <button onClick={() => setActiveTab('appearance')} className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'appearance' ? 'bg-primary text-on-primary shadow-lg' : 'text-outline hover:bg-surface-variant hover:text-on-surface'}`}><span className="material-symbols-outlined text-[16px]">palette</span>Visuals</button>
            <button onClick={() => setActiveTab('my-security')} className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'my-security' ? 'bg-primary text-on-primary shadow-lg' : 'text-outline hover:bg-surface-variant hover:text-on-surface'}`}><span className="material-symbols-outlined text-[16px]">shield_person</span> My Security</button>
            <button onClick={() => setActiveTab('ai-engine')} className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'ai-engine' ? 'bg-primary text-on-primary shadow-lg' : 'text-outline hover:bg-surface-variant hover:text-on-surface'}`}><span className="material-symbols-outlined text-[16px]">magic_button</span> AI Engine</button>
            <button onClick={() => setActiveTab('health')} className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'health' ? 'bg-primary text-on-primary shadow-lg' : 'text-outline hover:bg-surface-variant hover:text-on-surface'}`}><span className="material-symbols-outlined text-[16px]">monitor_heart</span> Diagnostics</button>
            
            {userIsAdmin && (
              <>
                <div className="pt-6 pb-2 px-4 text-[10px] font-black text-outline uppercase tracking-widest border-t border-outline-variant mt-4">Administrator</div>
                <button onClick={() => setActiveTab('admin-users')} className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'admin-users' ? 'bg-secondary-container text-on-secondary-container shadow-lg' : 'text-outline hover:bg-surface-variant hover:text-on-surface'}`}><span className="material-symbols-outlined text-[16px]">group</span> User Base</button>
                <button onClick={() => setActiveTab('admin-cloud')} className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'admin-cloud' ? 'bg-tertiary-container text-on-tertiary-container shadow-lg' : 'text-outline hover:bg-surface-variant hover:text-on-surface'}`}><span className="material-symbols-outlined text-[16px]">cloud</span> Cloud Setup</button>
                <button onClick={() => setActiveTab('admin-logs')} className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'admin-logs' ? 'bg-error-container text-on-error-container shadow-lg' : 'text-outline hover:bg-surface-variant hover:text-on-surface'}`}><span className="material-symbols-outlined text-[16px]">receipt_long</span> System Logs</button>
              </>
            )}
          </div>

          {/* Content */}
          <div className="flex-1 p-8 overflow-y-auto custom-scrollbar bg-surface-dim">
            
            {activeTab === 'appearance' && (
              <div className="space-y-8 animate-[fadeIn_0.2s_ease-out]">
                <div className="flex items-center justify-between p-6 bg-surface-container-low rounded-2xl border border-outline-variant">
                  <div>
                    <h4 className="font-bold text-on-surface uppercase tracking-tight">Theme Selector</h4>
                  </div>
                  <button onClick={toggleDarkMode} className={`w-14 h-7 rounded-full transition-all relative ${darkMode ? 'bg-primary' : 'bg-surface-variant'}`}>
                    <div className={`absolute top-1 w-5 h-5 bg-on-primary rounded-full shadow-md transition-all ${darkMode ? 'right-1' : 'left-1'}`} />
                  </button>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
                    {(['default', 'ocean', 'forest', 'sunset', 'rose', 'midnight', 'coffee', 'neon', 'cyberpunk', 'nord', 'dracula', 'lavender', 'earth', 'yellow', 'hyperblue'] as Theme[]).map(t => (
                      <button key={t} onClick={() => setTheme(t)} className={`px-4 py-3 rounded-xl text-[10px] font-black border transition-all uppercase tracking-tighter ${theme === t ? 'border-primary bg-primary text-on-primary shadow-xl scale-[1.05]' : 'border-outline-variant text-outline hover:border-primary'}`}>
                        {t}
                      </button>
                    ))}
                </div>
              ...
              <div className="space-y-8 animate-[fadeIn_0.2s_ease-out]">
                <div className="p-6 bg-surface-container-low rounded-2xl border border-outline-variant">
                  <div className="flex justify-between items-end mb-4">
                    <div>
                      <h4 className="font-bold text-on-surface uppercase tracking-tight">AI Compute Quota</h4>
                      <p className="text-xs text-outline">Global usage budget per user session.</p>
                    </div>
                    <span className="text-2xl font-black text-primary">{dailyAIUsage} / {DAILY_REQUEST_LIMIT}</span>
                  </div>
                  <div className="w-full h-3 bg-surface-variant rounded-full overflow-hidden">
                    <div className="h-full bg-primary transition-all duration-1000" style={{ width: `${Math.min(100, (dailyAIUsage / DAILY_REQUEST_LIMIT) * 100)}%` }} />
                  </div>
                </div>

                <div>
                  <h4 className="text-xs font-black text-outline uppercase tracking-widest mb-4">Session Activity Log</h4>
                  <div className="space-y-2">
                    {aiLogs.length > 0 ? aiLogs.map((log: any) => (
                      <div key={log.id} className="p-3 bg-surface-container-lowest rounded-lg flex justify-between items-center text-xs border border-outline-variant">
                        <div className="flex gap-4">
                          <span className="text-outline font-mono">[{new Date(log.timestamp).toLocaleTimeString()}]</span>
                          <span className="text-primary font-bold uppercase">{log.action}</span>
                          <span className="text-on-surface-variant">{log.details}</span>
                        </div>
                      </div>
                    )) : <p className="text-sm text-outline italic">No AI activity recorded in this session.</p>}
                  </div>
                </div>
              </div>
            )}
