...
// Added missing default export for SettingsPanel component to resolve App.tsx import error.
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="w-48 border-r border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 p-4 space-y-1">
            <button onClick={() => setActiveTab('appearance')} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'appearance' ? 'bg-white dark:bg-slate-800 shadow-sm text-primary-600' : 'text-slate-500'}`}>Appearance</button>
            <button onClick={() => setActiveTab('data')} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'data' ? 'bg-white dark:bg-slate-800 shadow-sm text-primary-600' : 'text-slate-500'}`}>Data Management</button>
            {userIsAdmin && (
              <>
                <button onClick={() => setActiveTab('admin')} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'admin' ? 'bg-white dark:bg-slate-800 shadow-sm text-primary-600' : 'text-slate-500'}`}>User Admin</button>
                <button onClick={() => setActiveTab('security')} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'security' ? 'bg-white dark:bg-slate-800 shadow-sm text-primary-600' : 'text-slate-500'}`}>Security Logs</button>
                <button onClick={() => setActiveTab('logs')} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'logs' ? 'bg-white dark:bg-slate-800 shadow-sm text-primary-600' : 'text-slate-500'}`}>AI & System</button>
              </>
            )}
          </div>
          ...
          <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {(['default', 'ocean', 'forest', 'sunset', 'rose', 'midnight', 'coffee', 'neon'] as Theme[]).map(t => (
                      <button key={t} onClick={() => setTheme(t)} className={`px-2 py-2 rounded-lg text-[10px] font-bold border transition-all ${theme === t ? 'border-primary-600 bg-primary-50 text-primary-700' : 'border-slate-200 text-slate-500'}`}>{t.toUpperCase()}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'security' && (
              <div className="space-y-3">
                ...
                <div className="space-y-1.5 h-96 overflow-y-auto">
                  {auditLogs.map(log => (
