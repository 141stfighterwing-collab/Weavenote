...
${text}`;
    ...
    await onAddNote(rawSubmission, activeType, [], [], useAI, title, activeType === 'project' ? { 
        manualProgress, 
        isCompleted 
    } : undefined);
  ...
  return (
    <div className={`rounded-xl shadow-lg border p-1 mb-8 transition-all duration-300 ${getBackgroundColor()}`}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-1">
        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder={activeType === 'project' ? "Project Name..." : "Title (optional)"} className="w-full px-4 py-3 bg-transparent border-b border-slate-100 dark:border-slate-700 focus:outline-none font-bold text-lg text-slate-800 dark:text-white" />

        {/* Formatting Toolbar */}
        {activeType !== 'project' && activeType !== 'document' && activeType !== 'notebook' && (
          <div className="flex items-center gap-1 p-2 bg-slate-50 dark:bg-slate-900/50 border-b dark:border-slate-700 overflow-x-auto no-scrollbar">
              ...
              {/* FIXED: Added missing quote and closing tag content to repair the unclosed attribute that broke the JSX parser */}
                ...
                <div className="flex flex-col md:flex-row">
                    <div className={`flex-1 ${activeType === 'code' ? 'md:border-r border-slate-100 dark:border-slate-700' : ''}`}>
                        <textarea 
                          ...
                          value={text} 
                          onChange={(e) => setText(e.target.value)} 
                          placeholder={activeType === 'notebook' ? "Draft your entry here... This will be added to the notebook view below." : "Type your notes here... Paste structured lists or code blocks freely."} 
        ...
        <div className="flex items-center justify-between p-3 border-t dark:border-slate-700">
            <div className="text-[10px] text-slate-400 px-2 italic uppercase font-black tracking-widest">
              {activeType} mode
            </div>
