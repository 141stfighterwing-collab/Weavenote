...
const NoteInput: React.FC<NoteInputProps> = ({ 
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
      e.preventDefault();
      if (isProcessing || isIngesting) return;
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-1">
        ...
        <div className="flex flex-wrap items-center gap-1 p-2 bg-slate-50 dark:bg-slate-900/50 border-b dark:border-slate-700 relative">
            ...
            <div className="flex gap-0.5 border-r dark:border-slate-700 pr-2 mr-1 text-slate-600 dark:text-slate-300">
                <button onClick={() => execCommand('bold')} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded font-black text-xs px-2.5 outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Bold" title="Bold">B</button>
                <button onClick={() => execCommand('italic')} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded italic text-xs px-2.5 outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Italic" title="Italic">I</button>
                <button onClick={() => execCommand('underline')} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded underline text-xs px-2.5 outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Underline" title="Underline">U</button>
                <button onClick={() => execCommand('strikeThrough')} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded line-through text-xs px-2.5 outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Strikethrough" title="Strikethrough">S</button>
            </div>

            <div className="flex gap-0.5 border-r dark:border-slate-700 pr-2 mr-1">
                <button onClick={() => execCommand('insertUnorderedList')} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-[10px] font-black px-2 uppercase text-slate-600 dark:text-slate-300 outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Bullet List" title="Bullet List">Bullet</button>
                <button onClick={() => execCommand('insertText', '- [ ] ')} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-[10px] font-black px-2 uppercase text-primary-600 outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Task List" title="Task List">Task</button>
            </div>

            <div className="flex gap-0.5 border-r dark:border-slate-700 pr-2 mr-1 text-slate-600 dark:text-slate-300">
                <button onClick={() => { const sel = window.getSelection(); if (sel) { const range = sel.getRangeAt(0); const sup = document.createElement('sup'); sup.textContent = sel.toString(); range.deleteContents(); range.insertNode(sup); } }} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-[10px] font-black px-2 outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Superscript" title="Superscript">X²</button>
                <button onClick={() => { const sel = window.getSelection(); if (sel) { const range = sel.getRangeAt(0); const sub = document.createElement('sub'); sub.textContent = sel.toString(); range.deleteContents(); range.insertNode(sub); } }} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-[10px] font-black px-2 outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Subscript" title="Subscript">X₂</button>
            </div>
            ...
            <div className="flex gap-1 items-center relative">
                ...
                <button 
                  ...
                  className={`p-1.5 rounded flex items-center gap-2 px-3 text-[10px] font-black uppercase transition-all outline-none focus-visible:ring-2 focus-visible:ring-primary-500 ${isIngesting ? 'bg-indigo-400 text-white cursor-wait animate-pulse' : (activeType === 'document' ? 'bg-indigo-600 text-white shadow-lg scale-105' : 'hover:bg-white dark:hover:bg-slate-700 text-slate-500')}`}
                >
                ...
                <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-base outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Insert Emoji" title="Insert Emoji">😀</button>
                <button onClick={() => setShowColorPicker(!showColorPicker)} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-base outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Text Color & Highlight" title="Text Color & Highlight">🎨</button>
                <button onClick={() => setShowGifPicker(!showGifPicker)} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-base outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Insert GIF" title="Insert GIF">🎬</button>
                <button onClick={() => setShowImageDialog(!showImageDialog)} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-base outline-none focus-visible:ring-2 focus-visible:ring-primary-500" aria-label="Insert Image URL" title="Insert Image URL">🖼️</button>
            ...
            <div className="flex flex-wrap gap-1.5 p-3 bg-slate-50 dark:bg-slate-900 rounded-2xl border dark:border-slate-700 min-h-[48px] items-center">
                {tags.map(tag => (
                    <span key={tag} style={getTagStyle(tag)} className="px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-2">
                        #{tag} <button type="button" onClick={() => setTags(tags.filter(t => t !== tag))} className="hover:text-red-500 outline-none focus-visible:ring-1 focus-visible:ring-red-500 rounded" aria-label={`Remove tag ${tag}`}>✕</button>
                    </span>
                ))}
                <input 
