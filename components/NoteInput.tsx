...
const NoteInput: React.FC<NoteInputProps> = ({ 
  ...
  const handleKeyDown = (e: React.KeyboardEvent, useAI: boolean) => {
    if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
      if (useAI && (isProcessing || isGuest || isIngesting)) return;
      if (!useAI && (isProcessing || isIngesting)) return;
      e.preventDefault();
      handleAction(useAI);
    }
  };
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-1">
        <div className="flex items-center justify-between border-b dark:border-slate-700">
            <input 
              ...
              onKeyDown={(e) => handleKeyDown(e, !isGuest)}
        ...
        <div className="flex flex-wrap items-center gap-1 p-2 bg-slate-50 dark:bg-slate-900/50 border-b dark:border-slate-700 relative">
            ...
            <div className="flex gap-0.5 border-r dark:border-slate-700 pr-2 mr-1 text-slate-600 dark:text-slate-300">
                <button onClick={() => execCommand('bold')} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded font-black text-xs px-2.5" title="Bold (Ctrl+B)" aria-label="Bold">B</button>
                <button onClick={() => execCommand('italic')} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded italic text-xs px-2.5" title="Italic (Ctrl+I)" aria-label="Italic">I</button>
                <button onClick={() => execCommand('underline')} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded underline text-xs px-2.5" title="Underline (Ctrl+U)" aria-label="Underline">U</button>
            ...
            <div className="flex gap-1 items-center relative">
                ...
                <button onClick={() => setShowColorPicker(!showColorPicker)} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-base" title="Text & Highlight Color" aria-label="Text & Highlight Color">🎨</button>
        ...
        <div className="flex items-center justify-between p-4 border-t dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 rounded-b-2xl">
            ...
            <div className="flex gap-3">
                <button
                  onClick={() => handleAction(false)} disabled={isProcessing || isIngesting}
                  ...
                  title={isGuest ? "Save locally (Ctrl+Enter)" : "Save without AI processing"}
                  aria-label="Save Raw"
                >
                  Save Raw
                </button>
                <button 
                  ...
                  title={isGuest ? "Login required for AI synthesis" : "Synthesize with AI (Ctrl+Enter)"}
