      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-1">
        ...
        <div className="flex flex-wrap items-center gap-1 p-2 bg-slate-50 dark:bg-slate-900/50 border-b dark:border-slate-700 relative">
            ...
            <div className="flex gap-0.5 border-r dark:border-slate-700 pr-2 mr-1 text-slate-600 dark:text-slate-300">
                <button onClick={() => execCommand('bold')} title="Bold" aria-label="Bold" className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded font-black text-xs px-2.5">B</button>
                <button onClick={() => execCommand('italic')} title="Italic" aria-label="Italic" className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded italic text-xs px-2.5">I</button>
                <button onClick={() => execCommand('underline')} title="Underline" aria-label="Underline" className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded underline text-xs px-2.5">U</button>
                <button onClick={() => execCommand('strikeThrough')} title="Strikethrough" aria-label="Strikethrough" className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded line-through text-xs px-2.5">S</button>
            </div>

            <div className="flex gap-0.5 border-r dark:border-slate-700 pr-2 mr-1">
                <button onClick={() => execCommand('insertUnorderedList')} title="Bullet List" aria-label="Bullet List" className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-[10px] font-black px-2 uppercase text-slate-600 dark:text-slate-300">Bullet</button>
                <button onClick={() => execCommand('insertText', '- [ ] ')} title="Add Task Checkbox" aria-label="Add Task Checkbox" className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-[10px] font-black px-2 uppercase text-primary-600">Task</button>
            </div>
            ...
            <div className="flex gap-1 items-center relative">
                ...
                <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} title="Emoji Picker" aria-label="Emoji Picker" className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-base">😀</button>
                <button onClick={() => setShowColorPicker(!showColorPicker)} title="Text & Highlight Colors" aria-label="Text & Highlight Colors" className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded text-base">🎨</button>
                ...
                {showEmojiPicker && (
                    <div className="absolute top-full left-0 mt-1 p-2 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-xl shadow-2xl z-50 grid grid-cols-4 gap-1 min-w-[140px]">
                        {EMOJIS.map(e => <button key={e} onClick={() => { execCommand('insertText', e); setShowEmojiPicker(false); }} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-lg" title={`Insert ${e}`} aria-label={`Insert ${e}`}>{e}</button>)}
                    </div>
                )}
                {showColorPicker && (
            ...
            <div className="flex flex-wrap gap-1.5 p-3 bg-slate-50 dark:bg-slate-900 rounded-2xl border dark:border-slate-700 min-h-[48px] items-center">
                {tags.map(tag => (
                    <span key={tag} style={getTagStyle(tag)} className="px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-2">
                        #{tag} <button type="button" onClick={() => setTags(tags.filter(t => t !== tag))} className="hover:text-red-500" aria-label={`Remove tag ${tag}`}>✕</button>
                    </span>
                ))}
                <input 
        ...
        <div className="flex items-center justify-between p-4 border-t dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 rounded-b-2xl">
            ...
            <div className="flex gap-3">
                <button onClick={() => handleAction(false)} disabled={isProcessing || isIngesting} title="Save Raw (Ctrl+Enter)" className="px-6 py-2 rounded-full font-bold text-[10px] bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 uppercase tracking-widest border border-slate-200 dark:border-slate-600 hover:shadow-md transition-all active:scale-95 disabled:opacity-50">Save Raw</button>
                <button 
