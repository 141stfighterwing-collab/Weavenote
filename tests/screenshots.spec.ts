...
test.describe('Weavenote Screenshot Tests', () => {
  ...
  test('Dark Mode Toggle', async ({ page }) => {
    ...
    const darkModeToggle = page.locator('h4:has-text("Dark Mode")').locator('..').locator('..').locator('button');
  ...
  test('Documentation - Settings Entry Point', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const panel = document.querySelector('[class*="fixed inset-0"]') as HTMLElement;
      if (panel) {
